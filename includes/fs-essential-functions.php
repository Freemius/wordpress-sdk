...
if ( ! function_exists( 'fs_redirect' ) ) {
	...
	if ( ! function_exists( 'fs_sanitize_redirect' ) ) {
		 ...
		 *
		 * @param string $location __comment_missing__
		 *
	...
	if ( ! function_exists( 'fs_kses_no_null' ) ) {
		 ...
		 *
		 * @param string $string __comment_missing__
		 *
		 * @return string
		 * @since 1.0.0
		 */
...
// End region Core Redirect (copied from BuddyPress) -----------------------------------------
if ( ! function_exists( '__fs' ) ) {
	 ...
	 * @param string $key  __comment_missing__
	 * @param string $slug __comment_missing__
	 *
	 * @return string
	 * @since 1.1.4
 ...
 *
 * @author    Vova Feldman (@svovaf)
 * @param string      $sdk_relative_path __comment_missing__
...
function fs_apply_filter( $slug, $tag, $value ) {
	...
	return call_user_func_array('apply_filters', array_merge(array( 'fs_' . $tag . '_' . $slug ),
	array_slice( $args, 2 ) ));
}
