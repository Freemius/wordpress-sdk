<?php
    ...
    class FS_Garbage_Collector {
        ...
        private function clean_up() {
            ...
            $product_types = array( 'plugin', 'theme' );
            ...
            foreach( $instances as $instance ) {
                $products_to_skip_by_type_and_slug[ $instance->get_module_type() ] = $instance->get_slug();
            }

            $_accounts = FS_Options::instance( WP_FS__ACCOUNTS_OPTION_NAME, true );

            $products_slugs_by_type = array_fill_keys( $product_types, array() );
            ...
            foreach( $products_slugs_by_type as $product_type => $product ) {
                ...
                foreach( $products_data_by_type[ $product_type ] as $products_data ) {
                    ...
                    foreach( $products_data as $option_name => $product_data ) {
                        $update = false;

                        if ( isset( $product_data[ $slug ] ) ) {
                            if ( fs_ends_with( $option_name, 'sites' ) ) {
                                $site = $product_data[ $slug ];
                            ...
                            $update = true;
                        } else if ( isset( $product_data[ "{$slug}:{$product_type}" ] ) ) {
                            unset( $product_data[ "{$slug}:{$product_type}" ] );
                            $update = true;
                        } else if ( isset( $product_data[ $product_id ] ) ) {
                            unset( $product_data[ $product_id ] );
                            $update = true;
                        } else if ( isset( $product_data[ $product_file ] ) ) {
                            unset( $product_data[ $product_file ] );
                            $update = true;
                        } else if ( in_array( $option_name, array( "all_{$product_type}s", "active_{$product_type}s" ) ) ) {
                            if ( isset( $product_data[ "{$product_type}s" ][ $product_file ] ) ) {
                                unset( $product_data[ "{$product_type}s" ][ $product_file ] );
                                $update = true;
                            }
                        }

                        if ( ! $update ) {
                            $_accounts->set_option( $option_name, $product_data, true );
                        }
                    }
                }
            }
