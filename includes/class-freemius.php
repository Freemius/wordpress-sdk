			...
			$uninstall_reasons = array(
				...
				'short-term'                                  => array(
					...
					array(
						...
						'text'              => sprintf( __fs( 'reason-great-but-need-specific-feature', $this->_slug ), $module_type ),
						'input_type'        => 'textarea',
						...
						$message = sprintf(
							...
							sprintf(
								...
								sprintf(
									...
									sprintf( __fs( 'curl-missing-sysadmin-desc', $this->_slug ), $this->_module_type )
								),
								sprintf(
									'<a href="%s"><b>%s</b></a>%s',
...
//				'secret_key' => $secret_key,
		...
		function _plugin_code_type_changed() {
			...
			if ( $this->is_premium() ) {
				...
				$this->_admin_notices->add_sticky(
					sprintf( __fs( 'premium-activated-message', $this->_slug ), $this->_module_type ),
					'premium_activated',
...
//			$this->sync_install( array(), true );
		...
		static function is_ajax_action_static( $actions, $module_id = null ) {
			...
			if ( is_array( $actions ) && 0 < count( $actions ) ) {
				...
				foreach ( $actions as $action ) {
					if ( $ajax_action === self::get_ajax_action_tag_static( $action, $module_id ) ) {
						return true;
					}
				}
			}
		...
		function _get_admin_page_url( $page = '', $params = array() ) {
			...
			if ( $this->_menu->is_cpt() ) {
				 ...
				 */
				$menu_slug = $this->has_settings_menu() ?
...
//			}
		...
		private function get_ajax_action_tag( $tag ) {
			return self::get_ajax_action_tag_static( $tag, $this->_module_id );
		}
		 ...
		 * @return string
		 */
		private static function get_ajax_action_tag_static( $tag, $module_id = null ) {
			$action = "wp_ajax_fs_{$tag}";
		...
		static function add_ajax_action_static(
			...
			add_action(
				self::get_ajax_action_tag_static( $tag, $module_id ),
				$function_to_add,
				$priority,
				0
			);
		...
		private function _sync_plugin_license( $background = false ) {
			...
			if ( $this->has_paid_plan() ) {
				switch ( $plan_change ) {
					...
					case 'downgraded':
						$this->_admin_notices->add_sticky(
							sprintf( __fs( 'license-expired-blocking-message', $this->_slug ), $this->_module_type ),
							'license_expired',
							__fs( 'hmm', $this->_slug ) . '...'
						);
		...
		function start_trial( $plan_name = false ) {
			...
			if ( $this->is_trial() ) {
				...
				$this->_admin_notices->add(
					sprintf( __fs( 'in-trial-mode', $this->_slug ), $this->_module_type ),
					__fs( 'oops', $this->_slug ) . '...',
					'error'
				);
			...
			if ( false !== $plan_name ) {
				...
				if ( ! $this->has_trial_plan() ) {
					...
					$this->_admin_notices->add(
						sprintf( __fs( 'no-trials', $this->_slug ), $this->_module_type ),
						__fs( 'oops', $this->_slug ) . '...',
						'error'
					);
			...
			if ( ! $this->is_api_result_entity( $plan ) ) {
				...
				$this->_admin_notices->add(
					sprintf( __fs( 'unexpected-api-error', $this->_slug ), $this->_module_type )
						...
						switch ( $result->error->code ) {
							case 'user_exist':
								$this->_admin_notices->add(
									...
									sprintf( __fs( 'user-exist-message_ownership', $this->_slug ), $this->_module_type, '<b>' . $new_email . '</b>' ) .
									sprintf(
