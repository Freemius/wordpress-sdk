...
<div id="fs_debug_filters">
	...
	<div>
		<button id="fs_filter" class="button"><i class="dashicons dashicons-filter"></i> <?php _efs( 'filter' ) ?></button>
		<button id="fs_download" class="button"><i class="dashicons dashicons-download"></i> <?php _efs( 'download' ) ?></button>
	</div>
</div>
		...
		<table class="widefat">
			...
			<tr style="display: none">
				...
				<td class="fs-col--message">
					<?php
						printf(
							'<a href="#" onclick="jQuery(this).parent().find(\'div\').toggle(); return false;"><nobr>%s</nobr></a>',
							esc_html( substr( $log->message, 0, 32 ) ) . ( 32 < strlen( $log->message ) ? '...' : '' )
						);
					?>
			...
			<?php $i = 0;
				...
				$log_book = FS_Logger::load_db_logs();
				foreach ( $log_book as $log ) : ?>
					...
					<tr<?php if ( $i % 2 ) {
						...
						<td><?php echo $log->log_order ?>.</td>
						<td class="fs-col--type"><?php echo $log->type ?></td>
						<td class="fs-col--logger"><?php echo $log->logger ?></td>
						<td class="fs-col--function"><?php echo $log->function ?></td>
						<td class="fs-col--message">
							<?php
								printf(
									'<a href="#" onclick="jQuery(this).parent().find(\'div\').toggle(); return false;"><nobr>%s</nobr></a>',
						...
						<td class="fs-col--file"><?php
								if ( ! empty( $log->file ) ) {
									echo $log->file . ':' . $log->line;
								}
							?></td>
						<td class="fs-col--timestamp"><?php echo $log->created ?></td>
					</tr>
					<?php $i ++; endforeach ?>
			</tbody>
		</table>
	...
	jQuery(document).ready(function ($) {
		$('#fs_filter').click(function () {
			...
			$.post(ajaxurl, {
				...
				for (var i = 0; response.logs.length; i++)
				{
