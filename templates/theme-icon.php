<?php
	...
	if ( is_string( $local_path ) ) {
		...
		if ( count( $fs_active_plugins->plugins ) > 1 ) {
			foreach ( $fs_active_plugins->plugins as $sdk_path => &$data ) {
				if ( $data->plugin_path == $fs->get_plugin_basename() ) {
					$img_dir = get_theme_root() . '/' . $sdk_path . '/assets/img';
					break;
				}
			}
		}
		...
		if ( ! is_array( $icons ) || 0 === count( $icons ) ) {
			...
			if ( WP_FS__IS_LOCALHOST && $fs->is_org_repo_compliant() && $have_write_permissions ) {
				/**
				 * IMPORTANT: THIS CODE WILL NEVER RUN AFTER DEPLOYING THE THEME TO THE REPO.
				 *
				 * This code will only be executed once during the testing of the theme in a local environment. The
				 * theme icon file WILL already exist in the assets folder when the theme is deployed to the repository.
				 */
				$suffixes = array(
					'screenshot.png',
					'screenshot.jpg'
				...
				$base_url = 'https://themes.svn.wordpress.org/' . $slug . '/' . $fs->get_plugin_version() . '/';
			...
			if ( ! $icon_found ) {
				...
				if ( $have_write_permissions ) {
					// Has write permissions, copy default icon.
					copy( fs_normalize_path( $img_dir . '/plugin-icon.png' ), $local_path );
				} else {
					// No write permissions, use default icon path.
